...
if __name__ == '__main__':
    ...
    with open('/opt/atlas/iceberg_segment/pkls/helheim_stac_items.pkl', 'rb') as f:
        helheim_stack_list = pickle.load(f)
    
    grid_path = '/opt/atlas/iceberg_segment/helheim/geoms/melange_box_grid_utm24.shp'
    ...
    for time in image_stac.time:
        ...
        with mem_raster(rgb_norm_raster, **profile_rgb) as ds:
            for i, geom in enumerate(grid.geometry): 
                ...
                out_path = f"/opt/atlas/iceberg_segment/helheim/{date}"
