...
uwSA_depth_mean = uwSA_xr_data_arr.mean(dim='length')
...
ax[0].axhline(y=Aww_depth,linewidth=3, color='#d62728')

ax[0].set_ylim(600,0)

ax[0].set_ylabel('Depth' , size=12)
ax[0].set_xlabel('Q$_{ib}$ (W)' ,size=12)


ax[1].plot(uwSA_depth_mean.data, uwSA_depth_mean.Z, c='black', lw=5)
ax[1].plot(uwSA_depth_mean.data, uwSA_depth_mean.Z, color='tab:blue',lw=3)
ax[1].axhline(y=Aww_depth,linewidth=3, color='#d62728')
ax[1].set_xlabel('Surface Area (m$^{2}$)' ,size=12)
...
fig.savefig(f'{op}Qib_depth.png',dpi=300)
