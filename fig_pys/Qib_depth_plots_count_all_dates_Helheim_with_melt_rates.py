...
for time_idx, berg_model_file in enumerate(berg_model_list):
    ...
    ax[0].set_xscale('log')
    ax[0].set_xlabel('Q$_{ib}$ (W)\nper Unit Depth' ,size=labelsize)
    ...
    ax[1].set_xscale('log')
    ax[1].set_xlabel('Surface Area  (m$^{2}$)\nper Unit Depth' , size=labelsize)
...
def get_xr_das(berg_model_list):
    ...
    return Qib_depth_sum, uwSA_depth_sum, uwSA_depth_sum_aw, total_melt_depth_sum, melt_per_day
    
    

op = '/media/laserglaciers/upernavik/iceberg_py/figs/'
fig.savefig(f'{op}Qib_melt_rates.png',dpi=300, bbox_inches='tight')
