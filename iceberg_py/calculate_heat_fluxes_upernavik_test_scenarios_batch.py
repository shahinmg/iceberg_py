...
gdf_pkl_path = '/media/laserglaciers/upernavik/iceberg_py/outfiles/upernavik/iceberg_geoms/keel_depths/'
gdf_list = [gdf for gdf in os.listdir(gdf_pkl_path) if gdf.endswith('gpkg')]
...
for berg_file in gdf_list:
    ...
    op_berg_model = '/media/laserglaciers/upernavik/iceberg_py/outfiles/upernavik/berg_model/'
    if not os.path.exists(op_berg_model):
        os.makedirs(op_berg_model)
    out_file = f'{op_berg_model}{date_str}_bergs_v2.pkl'
    ...
    tf_test = [ mean_temp - (2*temp_std), mean_temp, mean_temp + (2*temp_std)] # from histogram of TF from Slater
    ...
    for u_rel, constant_tf in u_rel_tf_zip:
        ...
        op = f'/media/laserglaciers/upernavik/iceberg_py/outfiles/upernavik/Qib/{date_str}/u_rel_{urel_str}_factor{factor}_melt_rates/'
        if not os.path.exists(op):
            os.makedirs(op)
        
        Q_ib_ds.to_netcdf(f'{op}{date_str}_upernavik_coeff_{factor}_constant_tf_{constant_tf}_constant_UREL_{u_rel}.nc')
