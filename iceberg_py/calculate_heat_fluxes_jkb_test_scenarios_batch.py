...
gdf_pkl_path = '/media/laserglaciers/upernavik/iceberg_py/outfiles/jkb/iceberg_geoms/dim_with_bin/'
...
for berg_file in gdf_list:
    ...
    op_berg_model = '/media/laserglaciers/upernavik/iceberg_py/outfiles/jkb/berg_model/'
    ...
    tf_test = [3.84, 4.35, 4.86] # from histogram of TF from Slater
    ...
    for u_rel, constant_tf in u_rel_tf_zip:
        ...
        out_file = f'{op_berg_model}{date_str}_urel{u_rel}_TF{constant_tf}_bergs_v2.pkl'
        ...
        op = f'/media/laserglaciers/upernavik/iceberg_py/outfiles/jkb/Qib/{date_str}/u_rel_{urel_str}_factor{factor}_melt_rates/'
        if not os.path.exists(op):
            os.makedirs(op)
        
        Q_ib_ds.to_netcdf(f'{op}{date_str}_jkb_coeff_{factor}_constant_tf_{constant_tf}_constant_UREL_{u_rel}.nc')
